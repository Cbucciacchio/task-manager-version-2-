...
def reg_user():
    '''This function helps the user register a new user for the admin only '''
...
def add_task():
    '''This function helps the user add a new task for themselves or another user '''
    new_task_user = input("Please enter the username of the person the task is assigned to: \n")
    while new_task_user not in users:
        print("User not found. Please enter an existing user's name.")
        new_task_user = input("Please enter an existing user's name to give them a task: \n")
    ...
    new_task_duedate = datetime.strptime(new_task_duedate, '%Y %m %d').date().isoformat()
...
def view_all():
    '''this function helps the user view all the tasks'''
...
def view_mine(username):
    '''This function helps view the user's task(s)'''
    tasks = []
...
def edit_task(task_line):
    '''this function helps the user edit an existing task'''
    ...
    if task_done == "No":
        ...
        if edit_choice == "1":
            ...
            new_duedate = input("Enter a new due date in the following format YYYY MM DD or -1 to skip: ")
            if new_duedate != "-1":
...
def generate_reports():
    '''this function helps generate a report to view all the tasks in an external file'''
    # read all tasks
    with open('tasks.txt', 'r') as f:
        lines = f.readlines()
...
def display_statistics():
    '''This function displays statistics, including the total number of users and the total number of tasks'''
    ...
    with open('user_overview.txt', 'r') as f:
        ...
        total_users = int(user_stats[0].strip())
        total_tasks = int(user_stats[1].strip())

    if total_tasks == 0:
        print("No tasks have been assigned to users.")
        return
...
def admin_menu():
    '''this function defines the admin menu with specific functions that are only allowed to use and see for the admin and not any users '''
    while True:
        print("\nAdmin Menu:")
...
: ''').lower()
        ...
        if menu == '1': #adding a new task
            add_task()

        elif menu == '2': #viewing all the tasks
            view_all()

        elif menu == '3': #veiwing the tasks depending on the user that is logged in 
            view_mine(username)
